...
public:
    vector<bool> checkIfPrerequisite(int numCourses, vector<vector<int>> &prerequisites, vector<vector<int>> &queries)
    {
        ...
        while(!bfs.empty()){
            ...
            for (auto& p: graph[curr].pred){
                isReachable[p][curr] = true;
            }

            for (auto& c: graph[curr].child){
                graph[c].indeg--;
                graph[c].pred.push_back(curr);
                graph[c].pred.insert(graph[c].pred.end(), graph[curr].pred.begin(), graph[curr].pred.end());
                if (graph[c].indeg == 0){
                    bfs.push(c);
                }
            }
        ...
        vector<bool> ret(qsize);
        for (int i = 0; i < qsize; i++){
            ret[i] = isReachable[queries[i][0]][queries[i][1]];
        }
        return ret;
    }
private:
